...
async def selection(app: Client,
    ...
    def navigation_buttons():
        back = InlineKeyboardButton(emoji.LEFT_ARROW, callback_data=back_id)
        next = InlineKeyboardButton(emoji.RIGHT_ARROW, callback_data=next_id)
        ret = []
        if ns.page > 0:
            ret.append(back)
    ...
    async def _done(app: Client, callback_query: CallbackQuery):
        callback_query.message.delete(True)
        ns.done = True

    async def _cancel(app: Client, callback_query: CallbackQuery):
        ns.canceled = True
        ns.done = True

    async def _select_item(app: Client, callback_query: CallbackQuery):
        data = callback_query.data[20:]
        m = rid[data]
    ...
    app.add_handler(
        CallbackQueryHandler(_select_all, filters=exact_match(select_all_id)))
    app.add_handler(
